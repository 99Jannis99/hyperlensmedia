...
const TOAST_TIMEOUT = 30 * 60 * 1000; // 30 minutes in milliseconds
...
(function() {
  ...
  function shouldShowToast() {
    const lastShown = localStorage.getItem('absoluteNetworkToastLastShown');
    if (!lastShown) return true;
    
    const timeSinceLastShown = Date.now() - parseInt(lastShown);
    return timeSinceLastShown >= TOAST_TIMEOUT;
  }

  // Update last shown timestamp
  function updateLastShown() {
    localStorage.setItem('absoluteNetworkToastLastShown', Date.now().toString());
  }
  ...
  */
  ...
  if (!shouldShowToast()) {
    // Skip creating the toast if it shouldn't be shown
    console.log('Toast was shown recently, skipping...');
    return;
  }
  ...
  */
  ...
  function styleCancelImg() {
    ...
    moduleCancelImgFunc.addEventListener("click", () => { 
      ...
      updateLastShown(); // Update timestamp when toast is closed
    });
  ...
  */
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      updateLastShown(); // Update timestamp when toast is shown
    }, 100);
  }, 0);
  ...
  */
  ...
  moduleContainerCSS.addEventListener("touchend", e => {
    if (touchStartY - 50 > touchMoveY) {
      ...
      updateLastShown(); // Update timestamp when toast is closed via touch
    }
